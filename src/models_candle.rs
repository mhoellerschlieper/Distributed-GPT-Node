...
impl CandleLlamaModel {
    pub fn from_safetensors(
        ...
        let cfg_hf: llama::LlamaConfig =
            serde_json::from_slice(&cfg_bytes).map_err(|e| format!("config.json parse: {}", e))?;
        let config = cfg_hf.into_config(false); // kein Flash-Attn auf CPU
        ...
        let model = llama::Llama::load(vb, &config).map_err(|e| format!("Llama load: {}", e))?;
        let cache = llama::Cache::new(true, dtype, &config, &dev)
    ...
    if ids.len() < self.prev_len {
        self.cache = llama::Cache::new(true, self.dtype, &self.config, &self.dev)
    ...
    if self.prev_len == 0 {
        ...
        let out: Vec<f32> = match rank {
            3 => {
                ...
                let last = logits
                    .narrow(1, t.saturating_sub(1), 1).map_err(|e| e.to_string())?
        ...
        let rows: Vec<Vec<f32>> = match rank {
            3 => {
                ...
                let last = logits
                    .narrow(1, t.saturating_sub(1), 1).map_err(|e| e.to_string())?
                    .squeeze(1).map_err(|e| e.to_string())?;
                last.to_vec2().map_err(|e| format!("to_vec2 (rank3 replay): {}", e))?
            }
            2 => logits.to_vec2().map_err(|e| format!("to_vec2 (rank2 replay): {}", e))?,
